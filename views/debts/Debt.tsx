...
export default function Debt() {
    ...
    return (
        <View style={{ backgroundColor: '#FFF', height: '100%' }}>
            <ModalWindow visible={visible} setVisible={setVisible} buttonTextLeft="Должны мне" buttonTextRight="Должен я" activeModalButton={activeModalButton} setActiveModalButton={setActiveModalButton} colorActiveLeft='#3FDEAE' colorActiveRight='#3FDEAE' functionCancelButton={()=>{setText(['','','',''])}} functionSaveButton={tryToSave}>
                ...
                <InputDate functionDate={()=>{setDatePickerVisible(true)}} textName='Крайняя дата' value={selectedDate.toString()} setValue={() => {setSelectedDate}} placeholder='Введите дату' keyboardType="default" colorActiveInput='##3FDEAE'/>
                <Input textName='Контакт' value={text[2].toString()} setItems={setText} index={2} placeholder='Введите контакт для связи (не обязательно)' keyboardType='phone-pad' colorActiveInput='#3FDEAE'/>
                ...
                <PickerBlock>
                    <Text style={{fontSize: 15, textAlign: 'center', width: 'auto', marginLeft: 55, textAlignVertical: 'center'}}>Счёт</Text>
                    <DropDownPicker open={openPicker} value={pickerValue} setOpen={setOpenPicker} setValue={setPickerValue} items={items} setItems={setItems} containerStyle={{width: '66%', alignSelf: 'flex-end'}} placeholder="Выберите счёт" dropDownDirection='TOP'/>
                </PickerBlock>
                {isDatePickerVisible && (
            ...
            <Header name='Debt' style='1' functionLeft={()=>{}} functionRight={()=>{setVisible(true); setActiveModalButton(debtTome)}}/>
                            ...
                            <Card key={index} onPress={()=>{del(index)}}>
                                <View style={{flex: 1, flexDirection: 'column'}}>
                                    <View style={{flex:1, justifyContent: 'space-between', flexDirection: 'row'}}>
                                        ...
                                        <Text>{item.date}</Text>
                                        <Text>{items.filter((account)=>{
                                            return account.value == item.id_account.toString()
                                        })[0].label}</Text>
